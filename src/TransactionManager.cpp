...
public:
    bool addTransaction(string account_no,string check_number,string date,string description,string debit_credit);
...
bool TransactionManager::addTransaction(string account_no,string check_number,string date,string description,string debit_credit){
    ...
    try{
        ...
        if(!mrm.checkAccountExists(account_no)) throw runtime_error("Account Doesnt Exist"); 

        FileManager fm(filename);
        record_line = record_line + newtxnid + "|" + account_no + "|" + check_number + "|" + date + "|" + description + "|" + debit_credit; 
